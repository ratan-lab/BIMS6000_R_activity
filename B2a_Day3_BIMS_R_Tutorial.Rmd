---
title: "BIMS 6000 R Tutorial 2019"
author: Basel Al-Barghouthi
...
This file is subset of the data from a manuscript on biorxiv (https://www.biorxiv.org/content/10.1101/604058v1.full). Today, we will look at normalized expression values of genes from a diagnostic panel that is currently used to determine Prostate cancer (PCa) prognosis in men with low or favorable intermediate risk disease. Some data exists that suggests the tests may provide inconsistent risk estimates in the presence of multifocal disease. But the data you will look at will be used to explore if the expression values for genes are different in African American men compared to European American men. Caution needs to be excercised in clinical decision making, and the risk scores determined from these tests need to be re-evaluated if there are statistically significant differences between the expression based on ancestry.
...
```{r}
potato = exp_data %>% mutate(AZGP1_log = log10(AZGP1))
```
...
```{r}
### We start again with the command, the data frame, and the two variables we want to plot.
g = ggplot(exp_data, aes(CAPRASgroup,S1PR4))
### Next, we add the geom layer, we just rename our object g as the first line + a geom layer
g = g + geom_boxplot()
### Finally, we look at the plot!
g
...
Now that you know how to do a boxplot for one gene, you should try to create a figure which shows the boxplots for all genes in your tibble. Hint: Use facet_grid (as you did above) or facet_wrap. Similarly, try to figure out how you would do a t.test for each gene. You could use `filter` and do this one gene at a time, but is there a better way? 
...
#Challenge Problem!
Now, we have used plots for data exploration, but what about the scientific question? Is there a statistically significant difference in gene expression between African Americans (AAM) and European Americans (EAM)? We can use a t.test to find out (We can dicuss later whether t.test is the right statistical test for this question, but this more to illustrate that application of statistical tests on these data is relatively straightforward). The null hypothesis for this test is that there is no difference between the two groups. 

The challenge is to perform this test for ALL genes at the same time.

Hint 1: The following will perform a t-test for COL1A1 by race

```{r}
?t.test
t.test(COL1A1 ~ Race, data=exp_data)
```


Hint 2: Use facets to plot your in one plot. You may have to subset your data (ex. 8 genes per plot) so your figure doesn't get too busy. Use select() for this filtration.
