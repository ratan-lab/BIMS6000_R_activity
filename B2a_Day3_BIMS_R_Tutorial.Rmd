...
#Aakrosh: should we provide a small csv for this example? A stumbling point for new users could be reading in files, especially with and without headers.
```
...
```{r}
df = mtcars
#Aakrosh: Perhaps this is nitpicking, but maybe change "df" to somethinf like "cars_df", to emphasize the importance of clear variable/object names. 
```
...
df
#Aakrosh: I think if we're going to focus on Tidyverse, this would be a good place to explain %>%, as it might not be completely intuitive.
```
...
```{r}
head(df)
#Aakrosh: quickly introduce "tail"?
```
...
## (4) group_by
The group_by function allows you to group the rows based on the values in a particular column. For example, say you want to compare the horse power of each the cars that have different numbers of carburators. 
...
g = g + xlab("# of carburators") + ylab("miles per gallon")
### Finally, we look at the plot!
g
```
