...
Let us look at the first few lines of the dataframe. You can use 'head' to look at the beginning, and 'tail' to look at the end of the dataframe. Look at the help section for 'head' to see how you would control the number of rows you can see using the command.
...
Let us read the metadata information from the file. Remember in this case, the file does not have the names of the columns. So we will use set_colnames to set those. Note that we have to specify 'col_names = F' with 'read_tsv' . What would happen if we do not specify 'col_names = F'?

```{r}
# get the population information
pop = read_tsv('metadata.txt', col_names = F) %>%
  set_colnames(c('sample','population','human'))
```

Again, lets look at the dataframe. This time we will peek at the first 10 rows.

```{r}
head(pop, n = 10)
```

We have two dataframes; one specifying the raw data, and the other one specifying the associated metadata. Storing files this way makes sense, but when exploring and manipulating it is easier to work with a single dataframe that encapsulates all the data we are interested in. 

But before we attempt to combine the two dataframes, we should convert these dataframes to conform to the tenets of 'tidy data'. Please read the article https://www.jstatsoft.org/article/view/v059i10 to understand the basic idea and philosophy behind tidy data. 
...
Now look at the 'pop' dataframe. Do we need to do anything to it to 'clean' it?

### Creating a single dataframe

We will use an inner join to combine the 'clean_data' dataframe with the 'pop' dataframe
...
Let us look at the first few lines of the resulting dataframe. 
```{r}
head(df)
```
...
In this case, we will use a command that is not part of tidyverse (prcomp), but it is used very frequently. Therefore a verb 'tidy' has been introduced as part of the 'broom' package to clean up such analyses. You will find similar functions to clean up data structures from gene-expression analyses for example.
...
Here we plot the 1st and the 2nd principal component. Can you plot the 2nd and the 3rd instead?
...
as.data.frame(predictions) %>%
  rownames_to_column() %>%
  set_colnames(c('sample', 'PC1', 'PC2', 'PC3')) %>%
  inner_join(pop) %>%
  ggplot(aes(PC1, PC2,color = population)) + geom_point() 
```

As you can see, human samples clump together. But what if we get really close. Axes can be controlled using 'xlim' and 'ylim' commands.
...
as.data.frame(predictions) %>%
  ...
  set_colnames(c('sample', 'PC1', 'PC2', 'PC3')) %>%
  inner_join(pop) %>%
  ggplot(aes(PC1, PC2,color = population)) + geom_point() + xlim(-150,-75) + ylim(-20,0)
```

All right. So it does seem that Africans are moving away from the Neanderthals.

Now lets see if we can use some numbers to convince us of the above. This section follows the analysis that has been detailed in the wonderful blog post here: http://johnhawks.net/weblog/reviews/neandertals/pigmentation/neandertal-introgression-1000-genomes-style-2011.html. 
...
#### STATLAB and BIOCONNECTOR COURSES
http://data.library.virginia.edu/training/
  Intro to R is Thursday, 9/6 (10:00 - 11:30)
  ...
  and many more!!! (http://cal.hsl.virginia.edu)
...
### CONTACT INFO
Dr. Aakrosh Ratan ar7jq at virginia dot edu

Olivia Sabik ols5fg at virginia dot edu
